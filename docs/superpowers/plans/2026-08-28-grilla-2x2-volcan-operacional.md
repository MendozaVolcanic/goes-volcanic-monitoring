...
- ~~**Sin GeoTIFF al zoom de volcán**~~ — **SALDADA (2026-08-29)**: el tab viejo ofrecía PNG + GeoTIFF por producto y la grilla sólo un PNG compuesto. Los tres helpers de descarga se movieron de `live_viewer.py` a **`dashboard/exports.py`** (una vista no importa de otra vista para bajar un archivo — es lo que nos costó la deuda de `PRODUCTS`), y la grilla suma un `st.expander` **cerrado** con el par PNG + GeoTIFF de los 3 productos RAMMB. VOLCAT queda fuera a propósito: su panel es un PNG compuesto por SSEC, no tenemos el array georreferenciado detrás, y fabricarle un GeoTIFF sería inventar una georreferencia que no controlamos. De paso, el **radio del encuadre** quedó escrito en el nombre del archivo y sobre-impreso en la imagen (por producto y en la captura compuesta): con radio ajustable, dos capturas del mismo minuto eran indistinguibles, y estas imágenes van a informes.
