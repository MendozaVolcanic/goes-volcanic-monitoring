...
## Gotchas conocidos
- **Python version pin OBLIGATORIO en Streamlit Cloud**: `runtime.txt` con `python-3.12` en la raíz. Sin él, Streamlit Cloud usa la última versión disponible (3.14 en 2026) y dispara `KeyError: 'dashboard.style'` (o cualquier otro módulo del proyecto) durante `_load_unlocked` en hot-reloads. Es race condition del importlib de 3.14, no bug del código. Síntomas: app cae 1-3 segundos durante un redeploy y se recupera sola con un `🔄 Updated app!`. La auditoría 2026-05-01 descartó imports circulares y otros patrones; el único fix robusto es fijar a 3.12 (estable, soportado por todas las deps).
- **Lazy import de módulos pesados desde `dashboard/views/`** (NO obligatorio, sí defensivo): si un módulo de `src/` importa numpy/scipy/cartopy a nivel top-level, importarlo top-level desde una vista hace que esa vista cargue toda la cadena al primer render. Para módulos cuya carga es lenta o opcional (ej: `animation_cache`), preferir `from src.X import Y` *dentro* de las funciones que lo usan. Esto NO previene el KeyError de Python 3.14 (eso lo cubre runtime.txt) pero acelera el cold-start.
